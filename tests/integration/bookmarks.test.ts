...
describe('DELETE /bookmarks/:id', () => {
    it('should respond with status 401 if no token is given', async () => {
        const response = await server.delete('/bookmarks/index');
    ...
    it('should respond with status 401 if given token is not valid', async () => {
        ...
        const response = await server.delete('/bookmarks/index').set('Authorization', `Bearer ${token}`);
    ...
    describe('when token is valid', () => {
        it('should respond with status 404 when bookmark index not exist or not found', async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
        ...
        it('should respond with status 200', async () => {
            ...
            const response = await server.delete(`/bookmarks/${bookmark.index}`).set('Authorization', `Bearer ${token}`);
